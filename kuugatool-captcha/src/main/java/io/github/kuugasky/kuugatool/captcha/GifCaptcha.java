...
import io.github.kuugasky.kuugatool.core.img.gif.AnimatedGifEncoder;
...
public class GifCaptcha extends AbstractCaptcha {
    ...
    private BufferedImage graphicsImage(char[] chars, Color[] fontColor, char[] words, int flag) {
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            g2d.drawString(words[i] + StringUtil.EMPTY, x + (font.getSize() + m) * i, y);
        }
        g2d.dispose();
        return image;
    }
    ...
    private Color getRandomColor(int min, int max) {
        ...
        if (min > maxColorValue) {
            min = maxColorValue;
        }
        if (max > maxColorValue) {
            max = maxColorValue;
        }
        if (min < 0) {
            min = 0;
