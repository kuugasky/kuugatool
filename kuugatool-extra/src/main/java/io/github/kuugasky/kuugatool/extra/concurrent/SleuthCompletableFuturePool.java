 ...
 */
@Slf4j
public class SleuthCompletableFuturePool {

    public static final String SLEUTH_COMPLETABLE_FUTURE_EXECUTOR_SERVICE = "sleuthCompletableFutureExecutorService";
        ...
        if (null == executorService || executorService.isShutdown() || executorService.isTerminated()) {
            synchronized (SleuthThreadPool.class) {
                if (executorService == null || executorService.isShutdown() || executorService.isTerminated()) {
                    ...
                    ObjectUtil.requireNonNull(applicationContext, SLEUTH_COMPLETABLE_FUTURE_EXECUTOR_SERVICE + " import error.");
                    executorService = (ExecutorService) applicationContext.getBean(SLEUTH_COMPLETABLE_FUTURE_EXECUTOR_SERVICE);
                }
            }
        }
    ...
    public static CompletableFuture<Void> runAsync(Runnable runnable) {
        return CompletableFuture.runAsync(runnable, getPool());
    }
    ...
    public static <U> CompletableFuture<U> supplyAsync(Supplier<U> supplier) {
        return CompletableFuture.supplyAsync(supplier, getPool());
    }
     ...
     *                    此种情况应使用{@link SleuthCompletableFuturePool#thenCompose(CompletableFuture, Function)}
    ...
    public static <T, U> CompletableFuture<U> thenApplyAsync(CompletableFuture<T> previousOne, Function<? super T, ? extends U> fn) {
        return previousOne.thenApplyAsync(fn, getPool());
    }
    ...
    public static <T, U> CompletableFuture<U> thenComposeAsync(CompletableFuture<T> previousOne, Function<? super T, ? extends CompletionStage<U>> fn) {
        return previousOne.thenComposeAsync(fn, getPool());
    }
    ...
    /**
     * 同{@link SleuthCompletableFuturePool#thenAccept(CompletableFuture, Consumer)}，区别在于该方法执行用{@code EXECUTOR_SERVICE}开启新异步线程执行
     * <p>
     * 终阶有入参，无返回
     *
     * @param previousOne 上一个CompletableFuture
    ...
    public static <T> CompletableFuture<Void> thenAcceptAsync(CompletableFuture<T> previousOne, Consumer<? super T> action) {
        return previousOne.thenAcceptAsync(action, getPool());
    }

    /**
     * 同{@link SleuthCompletableFuturePool#thenAccept(CompletableFuture, Consumer)}，区别在于该方法执行用{@code EXECUTOR_SERVICE}或{@code ForkJoinPool}开启新异步线程执行
     * <p>
     * 终阶有入参，无返回
     *
     * @param previousOne 上一个CompletableFuture
    ...
    public static <T> CompletableFuture<T> exceptionallyAsync(CompletableFuture<T> future, Function<Throwable, ? extends T> fn) {
        return future.exceptionallyAsync(fn, getPool());
    }
    ...
    public static <T> CompletableFuture<T> exceptionallyComposeAsync(CompletableFuture<T> future, Function<Throwable, ? extends CompletionStage<T>> fn) {
        return future.exceptionallyComposeAsync(fn, getPool());
    }
    ...
    public static <T, U> CompletableFuture<U> handleAsync(CompletableFuture<T> future, BiFunction<? super T, Throwable, ? extends U> fn) {
        return future.handleAsync(fn, getPool());
    }
    ...
    public static <T> KuugaCompletableFuture<T> build() {
        return new KuugaCompletableFuture<>(getPool());
    }
