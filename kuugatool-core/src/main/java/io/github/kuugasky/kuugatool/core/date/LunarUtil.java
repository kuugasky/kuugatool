...
public final class LunarUtil {
    ...
    public LunarUtil(Date date) {
        int leapMonth;
        Date baseDate = DateUtil.of(1900, 1, 31);
        // 求出和1900年1月31日相差的天数
        int offset = (int) DateUtil.dayDifference(baseDate, date);
        ...
        int iYear1 = 1900;
        int i = 2050;
        for (iYear = iYear1; iYear < i && offset > 0; iYear++) {
        ...
        int i1 = 13;
        for (iMonth = 1; iMonth < i1 && offset > 0; iMonth++) {
            // 闰月
    ...
    public String toString() {
        return year + "年" + (leap ? "闰" : StringUtil.EMPTY) + CHINESE_NUMBER[month - 1] + "月" + getChinaDayString(day);
    }
