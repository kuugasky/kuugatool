...
public final class ClassLoaderUtil {
     ...
     * 包名分界符: '.'
     */
    private static final char PACKAGE_SEPARATOR = KuugaCharConstants.DOT;
    /**
     * 内部类分界符: '$'
     */
    ...
    public static Class<?> loadClass(String name, ClassLoader classLoader, boolean isInitialized) throws UtilException {
        ...
        } else if (name.startsWith(NON_PRIMITIVE_ARRAY_PREFIX) && name.endsWith(KuugaConstants.EN_SEMICOLON)) {
    ...
    public static Class<?> loadPrimitiveClass(String name) {
        ...
        if (StringUtil.hasText(name)) {
            name = name.trim();
            if (name.length() <= KuugaConstants.EIGHT) {
