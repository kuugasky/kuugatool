...
public final class CollStreamUtil {
    ...
    public static <E, T, U> Map<T, Map<U, E>> group2Map(Collection<E> collection, Function<E, T> key1, Function<E, U> key2) {
        if (CollectionUtil.isEmpty(collection) || key1 == null || key2 == null) {
            return Collections.emptyMap();
        }
        return collection
                .stream()
    ...
    public static <E, T> Set<T> toSet(Collection<E> collection, Function<E, T> function) {
        if (CollectionUtil.isEmpty(collection) || function == null) {
     ...
     * @param merge 合并的lambda，将同key的value1和value2合并成最终的类型,注意value可能为空的情况
     * @param <K>   map中的key类型
    ...
    public static <K, X, Y, V> Map<K, V> merge(Map<K, X> map1, Map<K, Y> map2, BiFunction<X, Y, V> merge) {
        ...
        Map<K, V> map = new HashMap<>(key.size());
        for (K t : key) {
            X x = map1.get(t);
            Y y = map2.get(t);
            V z = merge.apply(x, y);
