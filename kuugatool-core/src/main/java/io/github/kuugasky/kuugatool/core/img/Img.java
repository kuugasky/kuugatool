...
public class Img implements Serializable {
    ...
    public Img scale(float scale) {
        ...
        if (ImgUtil.IMAGE_TYPE_PNG.equals(this.targetImageType)) {
            ...
            // 缩放后的图片宽
            int width = BigDecimalUtil.multiply(Integer.toString(srcImg.getWidth(null)), scaleStr).intValue();
            // 缩放后的图片高
            int height = BigDecimalUtil.multiply(Integer.toString(srcImg.getHeight(null)), scaleStr).intValue();
            scale(width, height);
        }
        return this;
    }
    ...
    public Img scale(int width, int height) {
        ...
        double sx = BigDecimalUtil.divide(width + StringUtil.EMPTY, srcWidth + StringUtil.EMPTY).doubleValue();
        double sy = BigDecimalUtil.divide(height + StringUtil.EMPTY, srcHeight + StringUtil.EMPTY).doubleValue();
    ...
    public Img scale(int width, int height, Color fixedColor) {
        ...
        double heightRatio = BigDecimalUtil.divide(height + StringUtil.EMPTY, srcHeight + StringUtil.EMPTY).doubleValue();
        double widthRatio = BigDecimalUtil.divide(width + StringUtil.EMPTY, srcWidth + StringUtil.EMPTY).doubleValue();
    ...
    public Img round(double arc) {
        ...
        // 通过弧度占比计算弧度
        arc = BigDecimalUtil.multiply(arc, Math.min(width, height)).doubleValue();
    ...
    public Img stroke(Color color, Stroke stroke) {
        ...
        g.setColor(ObjectUtil.getOrElse(color, Color.BLACK));
        if (null != stroke) {
            g.setStroke(stroke);
        }
    ...
    public boolean write(File targetFile) throws IORuntimeException {
        final String formatName = FilenameUtil.getExtension(targetFile);
        if (StringUtil.hasText(formatName)) {
