...
import io.github.kuugasky.kuugatool.core.io.Resource;
import io.github.kuugasky.kuugatool.core.lang.Assert;
import io.github.kuugasky.kuugatool.core.number.NumberParseUtil;
 ...
 * 参考：<a href="http://blog.csdn.net/zhangzhikaixinya/article/details/8459400">...</a>
 *
 * @author Looly
 */
public class ImgUtil {
    ...
    public static void scale(Image srcImg, File destFile, float scale) throws IORuntimeException {
        Img.from(srcImg).setTargetImageType(FilenameUtil.getExtension(destFile)).scale(scale).write(destFile);
    }
    ...
    /**
     * 缩放图像（按高度和宽度缩放）<br>
     * 缩放后默认格式与源图片相同，图片本身比例不变，新图片大小为width&height，底色填充为color<br>
     * 图片不拉伸对齐上下边<br>
     * 无法识别原图片默认JPG
     *
    ...
    public static void scale(File srcImageFile, File destImageFile, int width, int height, Color fixedColor) throws IORuntimeException {
        Img.from(srcImageFile)
                .setTargetImageType(FilenameUtil.getExtension(destImageFile))
    ...
    public static void slice(Image srcImage, File descDir, int destWidth, int destHeight) {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                ...
                // 输出为文件
                File targetFile = FileUtil.createFile(descDir.getPath() + "_r" + i + "_c" + j + ".jpg");
                write(tag, targetFile);
            }
        }
    }
    ...
    public static void sliceByRowsAndCols(Image srcImage, File destDir, int rows, int cols) throws IOException {
        ...
        try {
            ...
            int destWidth = NumberParseUtil.parseInt(srcWidth, cols); // 每张切片的宽度
            int destHeight = NumberParseUtil.parseInt(srcHeight, rows); // 每张切片的高度
    ...
    public static void convert(File srcImageFile, File destImageFile) throws IOException {
        ...
        final String srcExtName = FilenameUtil.getExtension(srcImageFile);
        final String destExtName = FilenameUtil.getExtension(destImageFile);
        if (StringUtil.equalsIgnoreCase(srcExtName, destExtName)) {
            // 扩展名相同直接复制文件
            FileUtil.copyFile(srcImageFile, destImageFile, true);
        }
    ...
    /**
     * 彩色转为黑白二值化图片，根据目标文件扩展名确定转换后的格式<br>
     * 黑白进阶
     *
     * @param srcImageFile  源图像地址
     * @param destImageFile 目标图像地址
    ...
    /**
     * 旋转图片为指定角度<br>
     * 来自：<a href="http://blog.51cto.com/cping1982/130066">...</a>
     *
     * @param image  目标图像
     * @param degree 旋转角度
     * @return 旋转后的图片
    ...
    public static BufferedImage copyImage(Image img, int imageType, Color backgroundColor) {
        ...
        final Graphics2D bGr = GraphicsUtil.createGraphics(bufferedImage, backgroundColor);
     ...
     * 存在类似(data:image/png;base64,)前缀自动移除
     *
    ...
    public static BufferedImage toImage(String base64) throws IORuntimeException {
        String base64Text = base64;
        if (base64Text.startsWith("data:image") && base64Text.contains(",")) {
            base64Text = base64.substring(base64.indexOf(",") + 1);
        }
        return toImage(Base64.getDecoder().decode(base64Text));
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static BufferedImage toImage(File file) throws IORuntimeException {
        return read(new ByteArrayInputStream(IoUtil.fileToByte(file)));
    }
    ...
    public static ByteArrayInputStream toStream(Image image, String imageType) {
        return IoUtil.toInputStream(toBytes(image, imageType));
    }
    ...
    public static void write(Image image, File targetFile) throws IORuntimeException {
        ...
        try {
            ...
            write(image, FilenameUtil.getExtension(targetFile), out);
        } finally {
            IoUtil.close(out);
        }
    }
    ...
    public static BufferedImage read(String imageFilePath) {
        return read(FileUtil.createFile(imageFilePath));
    }
     ...
     * @since 3.1.2
     */
    public static ImageInputStream getImageInputStream(File file) throws IORuntimeException {
        FileInputStream in = IoUtil.toInputStream(file);
        ImageInputStream result;
        try {
    ...
    public static Color hexToColor(String hex) {
        return getColor(Integer.parseInt(StringUtil.removeStart(hex, "#"), 16));
    }
    ...
    public static Color getColor(String colorName) {
        ...
        if ("BLACK".equals(colorName)) {
            ...
            // rgb值
            final List<String> rgb = Arrays.asList(colorName.split(","));
            if (3 == rgb.size()) {
                final Integer r = NumberParseUtil.parseInt(rgb.get(0));
                final Integer g = NumberParseUtil.parseInt(rgb.get(1));
                final Integer b = NumberParseUtil.parseInt(rgb.get(2));
